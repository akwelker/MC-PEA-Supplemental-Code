...
raw_data = pd.read_csv('1-dof-experimental_data/lifting3_27_2.csv')
...
for test_num in test_numbers:
    ...
    current = test_data['Desired Torque'].to_numpy()
...
# Add in the new data for the 99% cogging torque test

# Load the data for feedback linearization steps
raw_data = pd.read_csv('1-dof-experimental_data/lifting3_28.csv')
raw_data.columns = ['time', 'Motor Abs Position', 'Motor Abs Velocity', 'Accel 1', 'Accel 2', \
                            'Accel 3', 'Motor Inc Position', 'Motor Inc Velocity', 'Joint Position', 'Joint Velocity',
                            'Desired Torque', 'Desired Current', 'Actual Current', 'var 6', 'var 7', "Motor Temperature",
                            'var 8', 'Motor Voltage', 'var 9', 'var 10', 'var 11', 'test']
...
test_numbers.append(1)
for i in range(len(test_numbers)):

    ax.plot(times[i] - 4, joint_positions[i] - 60, label= str(round(percent_cogging_torques[i])) + '% Cogging Torque Disturbance', color=colors[i])
...
for i in range(len(test_numbers)):
    ax.plot(times[i] - 4, currents[i], label= str(round(percent_cogging_torques[i])) + '% Cogging Torque Disturbance', color=colors[i])
...
plt.savefig('figures/30_step_fig.png')

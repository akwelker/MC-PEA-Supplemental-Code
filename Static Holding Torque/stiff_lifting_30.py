...
raw_data = pd.read_csv('1-dof-experimental_data/lifiting_5_22.csv')
...
for i in range(len(test_numbers)):
    ...
    ax.plot(times[i], joint_positions[i] + 30, label= str(round(percent_cogging_torques[i])) + '% Cogging Torque Disturbance', color=colors[i])
...
fig.suptitle('MC-PEA performance while picking up various disturbance loads \n 30 Degree Step, Stiff Gains', fontsize=16)

plt.savefig('figures/30_step_stiff.png')
